package com.uikit.loader.avchat;
...
//            ageTv.setText(userInfo.getBirthday());
    ...
    public void showNetworkCondition(int grade)
    {
        if (grade >= 0 && grade < NETWORK_GRADE_DRAWABLE.length)
        {
            ...
            Drawable drawable = LoaderApp.CONTEXT.getResources().getDrawable(NETWORK_GRADE_DRAWABLE[grade]);
            if (drawable != null)
            {
    ...
    {
        if (show && !NetworkUtil.isWifi(LoaderApp.CONTEXT))
        {
    ...
    {
        int viewId = v.getId();
        if (viewId == R.id.avchat_audio_hangup)
        {
            listener.onHangUp();
        }
        else if (viewId == R.id.refuse)
        {
            listener.onRefuse();
        }
        else if (viewId == R.id.receive)
        {
            listener.onReceive();
        }
        else if (viewId == R.id.avchat_audio_mute)
        {
            listener.toggleMute();
        }
        else if (viewId == R.id.avchat_audio_speaker)
        {
            listener.toggleSpeaker();
        }
        else if (viewId == R.id.avchat_audio_switch_video)
        {
            if (isInSwitch)
            {
        ...
        else if (viewId == R.id.avchat_audio_record)
        {
